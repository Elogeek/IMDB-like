<div id="container">
    <div id="top">
        <div>
            <img src="../asset/img/logo.png" alt="photo site" id="logoMenu">
        </div>
        <div id="exit">X</div>
    </div>
    <div id="genre">
        <div id="firstRow">
            <div id="news">
                <h1><i class="fas fa-film"></i>Nouveautés</h1>
                <ul>
                    <li><a href="#">Dernières sorties manga</a></li>
                </ul>
            </div>

            <div id="tvShow">
                <h1><i class="far fa-newspaper"></i>Actualités</h1>
                <ul>
                    <li><a href="#"><i class="far fa-calendar-alt"></i>Agenda des sorties</a></li>
                    <li><a href="#">Manga à venir</a></li>
                    <li><a href="#">Comment devenir mangaka </a></li>
                </ul>
            </div>

            <div id="awardAndEvent">
                <h1><i class="fas fa-trophy"></i>Prix et événement</h1>
                <ul>
                    <li><a href="#">Meilleur manga de l'année</a></li>
                    <li><a href="#">Meilleur mangaka de l'année</a></li>
                    <li><a href="#">Meilleur dessin du mois</a></li>
                    <li><a href="#">Autres évènements</a></li>
                </ul>
            </div>
        </div>

        <div id="secondRow">
            <div id="watch">
                <h1><i class="fab fa-youtube"></i>Catalogue</h1>
                <ul>
                    <li><a href="#">Action</a></li>
                    <li><a href="#">Aventure</a></li>
                    <li><a href="#">Comédie</a></li>
                    <li><a href="#">Horreur</a></li>
                    <li><a href="#">Fantaisy</a></li>
                    <li><a href="#">Science-fiction</a></li>
                </ul>
            </div>

            <div id="community">
                <h1><i class="fas fa-globe-americas"></i>Communauté</h1>
                <ul>
                    <li><a href="#">Aide</a></li>
                    <li><a href="#">A propros de nous</a></li>
                    <li><a href="#">Infos légales</a></li>
                    <li><a href="#">Forum</a></li>
                    <li><a href="#">Nous trouver</a></li>
                    <li><a href="#">Rejoindre notre équipe</a></li>
                </ul>
            </div>

            <div id="watchList">
                <h1>Ma watchlist</h1>
                <ul>
                    <li><a href="#"><i class="fas fa-heart"></i></a></li>
                </ul>
            </div>
        </div>
    </div>
</div>
